...
CREATE SCHEMA `fullstackphp` DEFAULT CHARACTER SET utf8 ;
...
CREATE TABLE `users_address` (
	...
	PRIMARY KEY (`id`),
	CONSTRAINT `user_id` FOREIGN KEY (`users_id`) REFERENCES `users` (`id`) ON UPDATE NO ACTION ON DELETE CASCADE
)
COLLATE='utf8_general_ci'
ENGINE=InnoDB ;

-- Populando tabela users_address
...
-- ALTERANDO NOME DA TABELA USERS_ADDRESS PARA ADRRESS
RENAME TABLE users_address TO address;

-- INSERINDO REGRA DE NEGÓCIO NA TABELA USERS
ALTER TABLE `users`
	ALTER `first_name` DROP DEFAULT,
	ALTER `last_name` DROP DEFAULT,
	ALTER `email` DROP DEFAULT;
ALTER TABLE `users`
	CHANGE COLUMN `first_name` `first_name` VARCHAR(255) NOT NULL AFTER `id`,
	CHANGE COLUMN `last_name` `last_name` VARCHAR(255) NOT NULL AFTER `first_name`,
	CHANGE COLUMN `email` `email` VARCHAR(255) NOT NULL AFTER `last_name`,
	ADD COLUMN `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER `document`,
	ADD COLUMN `updated_at` TIMESTAMP NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP AFTER `created_at`,
	ADD UNIQUE INDEX `email` (`email`);

	-- INSERINDO REGRA DE NEGÓCIO NA TABELA ADDRESS
ALTER TABLE `address`
	ALTER `street` DROP DEFAULT,
	ALTER `number` DROP DEFAULT;
ALTER TABLE `address`
	CHANGE COLUMN `street` `street` VARCHAR(255) NOT NULL AFTER `id`,
	CHANGE COLUMN `number` `number` VARCHAR(255) NOT NULL AFTER `street`,
	ADD COLUMN `created_at` TIMESTAMP NOT NULL DEFAULT CURRENT_TIMESTAMP AFTER `complement`,
	ADD COLUMN `updated_at` TIMESTAMP NULL DEFAULT NULL ON UPDATE CURRENT_TIMESTAMP AFTER `created_at`;
